...
SOURCES: list[dict[str, Any]] = [
    {
        "id": "unece-grva",
        "region": "UNECE",
        "name": "UNECE GRVA – UN Digital Library",
    ...
    },
    {
        "id": "unece-wp29",
        "region": "UNECE",
        "name": "UNECE WP.29 – UN Digital Library",
        "adapter": "un_digital_library",
...
def _undl_search(source: dict[str, Any], pattern: str, field: str = "") -> list[dict[str, Any]]:
    ...
    for record in records:
        item = _undl_record_to_raw(record, source)
        if item:
            out.append(item)
    return out
...
def _unece_wiki_items(source: dict[str, Any], url: str) -> list[dict[str, Any]]:
    ...
    for a in parser.anchors:
        ...
        if len(t) < 12 or not in_scope(t):
            continue
        # Keep only document-like entries from the official UNECE workspace.
...
def adapter_un_digital_library(source: dict[str, Any]) -> list[dict[str, Any]]:
    """UNECE/WP.29 monitoring via official machine-readable UN systems.

    Direct unece.org HTML currently rejects GitHub runners with HTTP 403. The
    UN Digital Library API is an official UN machine-to-machine interface and is
    therefore the primary source. The official UNECE Wiki is supplementary for
    current GRVA informal documents.
    """
    out: list[dict[str, Any]] = []
    successes = 0
    errors: list[tuple[str, Exception]] = []
    for spec in source.get("search_patterns", []):
        ...
        pseudo_url = "https://digitallibrary.un.org/search?" + urllib.parse.urlencode({"p": pattern, "f": field})
        try:
            out.extend(_undl_search(source, pattern, field))
            successes += 1
        except Exception as exc:
            errors.append((pseudo_url, exc))
    for url in source.get("wiki_urls", []):
        try:
            ...
            errors.append((url, exc))
    if not successes:
        raise RuntimeError("Official UN machine-readable sources could not be retrieved. " + error_summary(errors))
...
def collect_source(source: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        elif adapter == "un_digital_library":
            raw = adapter_un_digital_library(source)
