...
# Optional per-source success messages used when an official fallback path is
# required because a public HTML page blocks GitHub-hosted automation.
SOURCE_SUCCESS_MESSAGES: dict[str, str] = {}
...
SOURCES: list[dict[str, Any]] = [
    ...
    {
        "id": "unece-media-documents",
        "region": "UNECE",
        "name": "UNECE – Media / Documents",
        "adapter": "unece_media_documents",
        "page_url": "https://unece.org/media/documents",
        "urls": [
            "https://unece.org/media/documents",
            "https://unece.org/media/files-download?page=0",
            "https://unece.org/media/files-download?page=1",
            "https://unece.org/media/files-download?page=2",
            "https://unece.org/media/documents-download/events/411623",
            "https://unece.org/media/documents-download/events/411622",
            "https://unece.org/media/documents-download/events/408709",
        ],
        "fallback_search_terms": [
            "GRVA automated driving",
            "WP.29 automated driving",
            "GRVA driver assistance",
            "GRVA cybersecurity software update",
            "UN Regulation 185",
        ],
    },
    {
        "id": "unece-iwg-ai",
        "region": "UNECE",
        "name": "UNECE GRVA / WP.29 – IWG on Artificial Intelligence",
        "adapter": "unece_ai_iwg",
        "page_url": "https://wiki.unece.org/spaces/trans/pages/313393447/Artificial%2BIntelligence%2BAI",
        "root_urls": [
            "https://wiki.unece.org/spaces/trans/pages/313393447/Artificial%2BIntelligence%2BAI",
            "https://wiki.unece.org/spaces/trans/pages/917755/Home%2BVehicle%2BRegulations",
        ],
        "seed_urls": [
            "https://wiki.unece.org/spaces/trans/pages/408944647/AI%2B11th%2Bsession",
            "https://wiki.unece.org/spaces/trans/pages/362774847/AI%2B9th%2BSession",
            "https://wiki.unece.org/spaces/trans/pages/362774845/AI%2B8th%2BSession",
        ],
        "fallback_search_terms": [
            "IWG on AI GRVA",
            "Artificial Intelligence AI IWG GRVA",
            "Status Report AI IWG",
            "GRVA artificial intelligence vehicle",
        ],
        "max_sessions": 6,
    },
    {
    ...
    },
    {
        "id": "eu-dg-move",
        "region": "EU",
        "name": "European Commission DG MOVE – Mobility & Transport",
        "adapter": "official_pages",
        "page_url": "https://transport.ec.europa.eu/index_en",
        "include_policy": True,
        "urls": [
            ...
            "https://road-safety.transport.ec.europa.eu/what-we-do/eu-road-safety-legislation_en",
    ...
    },
    {
        "id": "eu-dg-connect",
        "region": "EU",
        "name": "European Commission DG CONNECT – Digital & Connected Mobility",
        "adapter": "official_pages",
        "page_url": "https://digital-strategy.ec.europa.eu/en/policies/connected-and-automated-mobility",
        "include_policy": True,
        "urls": [
            "https://digital-strategy.ec.europa.eu/en/policies/connected-and-automated-mobility",
            "https://digital-strategy.ec.europa.eu/en/policies/vehicle-alliance",
            "https://digital-strategy.ec.europa.eu/en/policies/digitalisation-mobility",
            "https://digital-strategy.ec.europa.eu/en/policies/technologies-digitalisation-transport",
            "https://digital-strategy.ec.europa.eu/en/news/ict-supply-chain-security-eu-adopts-toolbox-mitigate-risks",
            "https://digital-strategy.ec.europa.eu/en/news/autonomous-drive-ambition-cities-adacities-initiative-bring-autonomous-driving-technology-cities",
            "https://digital-strategy.ec.europa.eu/en/library/first-ecava-working-groups-demonstrate-industrys-willingness-collaborate-next-generation-digital",
    ...
    {
        "id": "eu-eurlex-legislation",
        "region": "EU",
        "name": "EUR-Lex – EU Legislation & Preparatory Acts",
        "adapter": "eurlex_general",
        "page_url": "https://eur-lex.europa.eu/homepage.html",
        "sparql_url": "https://publications.europa.eu/webapi/rdf/sparql",
        # Current Draft/In progress monitoring remains focused on the last year.
        # Relevant EU acts that are explicitly In force are retained as a
        # regulatory baseline back to 1 January 2022.
        "monitor_days": 365,
        "in_force_since": "2022-01-01",
        "max_results": 2500,
        # Direct CELEX watch list for core IAMTS files. These records are
        # resolved through CELLAR by CELEX even if thematic title indexing
        # temporarily misses them.
        "critical_celex": [
            "32022R1426",
            "32026R0481",
            "52025PC0179",
...
SCOPE_TERMS = [
    ...
    "periodic technical inspection", "periodic technical inspections", "technical roadside inspection",
    "vehicle inspection", "vehicle inspections", "pti directive", "roadworthiness directive",
    "vehicle registration", "vehicle registration data", "vehicle registration documents",
    "digital vehicle registration", "motor vehicle safety", "market surveillance", "eu type-approval",
    "software-defined vehicle", "software defined vehicle", "automotive ai", "ai in road vehicles",
    "iwg on ai", "informal working group on artificial intelligence", "artificial intelligence in the context of road vehicles",
...
OEIL_SCOPE_TERMS = [
    ...
    "roadworthiness", "roadworthiness package", "periodic technical inspection",
    "technical roadside inspection", "vehicle registration data", "vehicle registration documents",
    "digital vehicle registration", "market surveillance", "Regulation (EU) 2018/858",
...
OJ_SCOPE_TERMS = [
    ...
    "type approval of motor vehicles", "motor vehicle safety", "roadworthiness", "roadworthiness package",
    "periodic technical inspection", "technical roadside inspection", "vehicle registration data",
    "vehicle registration documents", "digital vehicle registration", "market surveillance",
...
POLICY_AND_REGULATORY_TERMS = REGULATORY_TERMS + [
    "policy", "policies", "legislation", "legislative", "strategy", "initiative", "action plan",
    "roadmap", "toolbox", "recommendation", "recommendations", "deployment", "alliance",
    "data act", "ai act", "cyber resilience", "cyber security", "cybersecurity", "market surveillance",
    "roadworthiness", "vehicle registration", "type-approval", "type approval", "technical inspection",
]
...
TOPIC_RULES = [
    ...
    ("Roadworthiness & Vehicle Data", ["roadworthiness", "periodic technical inspection", "vehicle registration data", "vehicle registration documents", "technical roadside inspection"]),
]
...
def normalize_item(raw: dict[str, Any], source: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        # Prevent older in-force EU reference acts from appearing as newly
        # adopted when the 2022 baseline is first loaded.
        "historicalBaseline": bool(raw.get("historicalBaseline", False)),
        **score,
    }
...
EURLEX_GENERAL_TERMS = [
    "roadworthiness", "roadworthiness package", "periodic technical inspection",
    "technical roadside inspection", "vehicle registration data", "vehicle registration documents",
    "digital vehicle registration", "vehicle type-approval", "vehicle type approval",
    "type-approval of motor vehicles", "type approval of motor vehicles", "automated driving",
    "automated driving system", "automated driving system of fully automated vehicles",
    "fully automated vehicle", "fully automated vehicles", "automated parking", "automated valet parking",
    "automated vehicle", "advanced driver assistance", "driver assistance",
    "connected vehicle", "vehicle cybersecurity", "vehicle cyber security", "vehicle software",
    "software update", "market surveillance of motor vehicles", "Regulation (EU) 2018/858",
    "Regulation (EU) 2019/2144", "UN Regulation No. 185", "UN R185",
]


def eurlex_general_in_scope(text: str) -> bool:
    return text_has_any(text, EURLEX_GENERAL_TERMS) or (
        in_scope(text) and text_has_any(text, [
            "vehicle", "motor vehicle", "driving", "roadworthiness", "transport", "type-approval",
            "type approval", "registration", "cybersecurity", "software update",
        ])
    )


def _eurlex_general_status(title: str, work_class: str, in_force: str, entry_date: str) -> str:
    flag = low(in_force)
    if flag in {"true", "1"}:
        return "In force"
    if "act_preparatory" in low(work_class):
        return "Draft"
    if entry_date:
        try:
            ...
            if entry > today:
                return "In progress"
            # A recorded past entry-into-force date is sufficient only where
            # CELLAR does not explicitly mark the act as no longer in force.
            if flag not in {"false", "0"}:
                return "In force"
        except ValueError:
            pass
    classified = classify_status(title)
    return classified if classified != "Status unclear" else "Status unclear"


def _eurlex_general_via_sparql(source: dict[str, Any]) -> list[dict[str, Any]]:
    end = datetime.now(timezone.utc).date()
    recent_start = end - timedelta(days=max(1, int(source.get("monitor_days", 365))) - 1)
    configured_baseline = parse_date_any(source.get("in_force_since", "2022-01-01")) or "2022-01-01"
    try:
        baseline_start = datetime.fromisoformat(configured_baseline).date()
    except ValueError:
        baseline_start = datetime(2022, 1, 1).date()
    query_start = min(recent_start, baseline_start)
    title_filters = [
        "roadworthiness", "periodic technical inspection", "technical roadside inspection",
        "vehicle registration", "type-approval", "type approval", "automated driving",
        "automated vehicle", "driver assistance", "connected vehicle", "vehicle cybersecurity",
        "vehicle software", "software update", "market surveillance of motor vehicles",
    ]
    filter_expr = " || ".join(
        f'CONTAINS(LCASE(STR(?title)), "{term}")' for term in title_filters
    )
    query = f'''\
PREFIX cdm: <http://publications.europa.eu/ontology/cdm#>
PREFIX owl: <http://www.w3.org/2002/07/owl#>
PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
SELECT DISTINCT ?work ?class ?sameAs ?celex ?title ?date ?inforce ?entry
WHERE {{
  ?work a ?class ;
        cdm:work_date_document ?date .
  FILTER (?date >= "{query_start.isoformat()}"^^xsd:date && ?date <= "{end.isoformat()}"^^xsd:date)
  ?expr cdm:expression_belongs_to_work ?work ;
        cdm:expression_uses_language <http://publications.europa.eu/resource/authority/language/ENG> ;
        cdm:expression_title ?title .
  FILTER ({filter_expr})
  OPTIONAL {{ ?work owl:sameAs ?sameAs . }}
  OPTIONAL {{ ?work cdm:resource_legal_id_celex ?celex . }}
  OPTIONAL {{ ?work cdm:resource_legal_in-force ?inforce . }}
  OPTIONAL {{ ?work cdm:resource_legal_date_entry-into-force ?entry . }}
}}
ORDER BY DESC(?date)
LIMIT {int(source.get("max_results", 2500))}
'''
    ...
    for b in bindings:
        ...
        if not title or not eurlex_general_in_scope(title):
            continue
        work = _binding_value(b, "work")
        celex = _binding_value(b, "celex")
        same_as = _binding_value(b, "sameAs")
        work_class = _binding_value(b, "class")
        date = parse_date_any(_binding_value(b, "date"))
        entry = parse_date_any(_binding_value(b, "entry"))
        in_force = _binding_value(b, "inforce")
        status = _eurlex_general_status(title, work_class, in_force, entry)

        # Keep Draft/In progress/unclear material focused on the recent
        # monitoring window. Older material is retained only when CELLAR
        # supports an In force classification and it dates from 2022 onward.
        doc_date = None
        if date:
            try:
                doc_date = datetime.fromisoformat(date).date()
            except ValueError:
                pass
        historical_baseline = bool(
            status == "In force" and doc_date and baseline_start <= doc_date < recent_start
        )
        if doc_date and doc_date < recent_start and not historical_baseline:
            continue
        if doc_date and doc_date < baseline_start:
            continue

        if celex:
            link = "https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:" + urllib.parse.quote(celex, safe="")
        elif "eur-lex.europa.eu" in same_as or "/eli/" in same_as or "/celex/" in same_as:
            link = same_as
        else:
            link = work or source["page_url"]
        class_name = work_class.rsplit("#", 1)[-1] if work_class else "EU document"
        summary = f"EUR-Lex/CELLAR {class_name.replace('_', ' ')} dated {date or 'within the monitoring period'}."
        if entry:
            summary += f" Recorded entry-into-force date: {entry}."
        if historical_baseline:
            summary += " Included in the IAMTS EU in-force regulatory baseline covering relevant acts from 1 January 2022."
        out.append({
            "key": celex or work or title,
            "title": title,
            "summary": summary,
            "date": date,
            "url": link,
...
def _eurlex_general_via_search_pages(source: dict[str, Any]) -> list[dict[str, Any]]:
    queries = [
        "roadworthiness", "vehicle registration data", "vehicle type approval",
        "automated driving", "advanced driver assistance", "vehicle cybersecurity",
    ]
    out: list[dict[str, Any]] = []
    success = 0
    errors: list[tuple[str, Exception]] = []
    for term in queries:
        url = "https://eur-lex.europa.eu/search.html?scope=EURLEX&lang=en&type=quick&text=" + urllib.parse.quote(term)
        try:
        ...
        for a in parser.anchors:
            title = norm(a.text)
            if len(title) < 20 or not eurlex_general_in_scope(title):
                continue
            if "/legal-content/" not in a.url and "CELEX" not in a.url.upper():
                continue
            out.append({
                "key": a.url,
                "title": truncate(title, 360),
                "summary": "Relevant EU legal or preparatory document identified through the official EUR-Lex search.",
                "date": extract_date(title),
                "url": a.url,
                "status": classify_status(title),
            })
    if not success:
        raise FetchFailure("EUR-Lex search pages could not be retrieved. " + error_summary(errors))
    return out


def _eurlex_direct_celex_records(source: dict[str, Any]) -> list[dict[str, Any]]:
    # Safety net for a small editable list of core IAMTS EU files. CELLAR is
    # queried directly by CELEX, so a temporary thematic-title-index gap cannot
    # hide an important act such as 32026R0481.
    celexes = [norm(x) for x in source.get("critical_celex", []) if norm(x)]
    if not celexes:
        return []
    baseline = datetime.fromisoformat(source.get("in_force_since", "2022-01-01")).date()
    recent_start = datetime.now(timezone.utc).date() - timedelta(days=max(1, int(source.get("monitor_days", 365))) - 1)
    values = ", ".join('"' + x.replace('"', '') + '"' for x in celexes)
    query = (
        "PREFIX cdm: <http://publications.europa.eu/ontology/cdm#>\n"
        "SELECT DISTINCT ?work ?class ?celex ?title ?date ?inforce ?entry\n"
        "WHERE {\n"
        "  ?work a ?class ; cdm:resource_legal_id_celex ?celex ; cdm:work_date_document ?date .\n"
        f"  FILTER (STR(?celex) IN ({values}))\n"
        "  ?expr cdm:expression_belongs_to_work ?work ;\n"
        "        cdm:expression_uses_language <http://publications.europa.eu/resource/authority/language/ENG> ;\n"
        "        cdm:expression_title ?title .\n"
        "  OPTIONAL { ?work cdm:resource_legal_in-force ?inforce . }\n"
    ...
    for b in bindings:
        celex = _binding_value(b, "celex")
        title = _binding_value(b, "title")
        if celex not in celexes or not title:
            continue
        date = parse_date_any(_binding_value(b, "date"))
        entry = parse_date_any(_binding_value(b, "entry"))
        status = _eurlex_general_status(title, _binding_value(b, "class"), _binding_value(b, "inforce"), entry)
        historical = False
        if status == "In force" and date:
            try:
                d = datetime.fromisoformat(date).date()
                historical = baseline <= d < recent_start
            except ValueError:
                pass
        summary = f"Direct EUR-Lex/CELLAR CELEX verification for {celex}."
        if entry:
            summary += f" Recorded entry-into-force date: {entry}."
        if historical:
            summary += " Included in the IAMTS EU in-force regulatory baseline from 1 January 2022."
        out.append({
            "key": celex,
            "title": title,
            "summary": summary,
            "date": date,
            "url": "https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:" + urllib.parse.quote(celex, safe=""),
            "status": status,
...
def adapter_eurlex_general(source: dict[str, Any]) -> list[dict[str, Any]]:
    main: list[dict[str, Any]] = []
    main_error: Exception | None = None
    try:
        main = _eurlex_general_via_sparql(source)
    except Exception as sparql_exc:
        main_error = sparql_exc
        try:
            main = _eurlex_general_via_search_pages(source)
        except Exception as search_exc:
            main_error = RuntimeError(f"CELLAR: {norm(sparql_exc)} | EUR-Lex search: {norm(search_exc)}")
    direct = _eurlex_direct_celex_records(source)
    if not main and not direct and main_error:
        raise RuntimeError(f"EUR-Lex legislation monitoring failed — {norm(main_error)}") from main_error
    merged: dict[str, dict[str, Any]] = {}
    for item in main + direct:
...
def adapter_official_pages(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for url in source["urls"]:
        try:
            ...
            errors.append((url, exc))
            continue
        match_terms = POLICY_AND_REGULATORY_TERMS if source.get("include_policy") else REGULATORY_TERMS
        for a in parser.anchors:
            t = norm(a.text)
            if len(t) < 18 or not in_scope(t) or not text_has_any(t, match_terms):
                continue
        ...
        page_text = html_to_text(html_text)
        if in_scope(page_text) and text_has_any(page_text, match_terms):
            for pattern in [
                r"([^.!?]{15,220}(?:automated vehicle|automated driving|driver assistance|connected vehicle|vehicle cybersecurity|software update|roadworthiness|vehicle registration|type-approval|type approval)[^.!?]{0,220})",
                r"([^.!?]{15,220}(?:AV Framework|AV safety|ADS|ADAS|ECAVA|ADACities|Roadworthiness Package)[^.!?]{0,220})",
            ]:
                m = re.search(pattern, page_text, re.I)
                if m:
                    candidate = norm(m.group(1))
                    if in_scope(candidate) and text_has_any(candidate, match_terms):
                        out.append({
...
def _unece_index_search_items(source: dict[str, Any], terms: list[str], ai_only: bool = False) -> tuple[list[dict[str, Any]], int, list[tuple[str, Exception]]]:
    # Official UNECE site-search fallback for public pages that block GitHub
    # runners. Results remain UNECE-hosted/indexed; no third-party data source
    # is introduced.
    out: list[dict[str, Any]] = []
    successes = 0
    errors: list[tuple[str, Exception]] = []
    for term in terms:
        url = "https://unece.org/search_content_unece?" + urllib.parse.urlencode({"keyword": term})
        try:
            html_text = fetch_text(url)
            parser = parse_html(html_text, url)
            successes += 1
            ...
            errors.append((url, exc))
            continue
        candidates = [(row.text, row.anchors) for row in parser.rows] + [(a.text, [a]) for a in parser.anchors]
        for text, anchors in candidates:
            t = norm(text)
            if len(t) < 12:
                continue
            link = next((a.url for a in anchors if "unece.org" in a.url or "wiki.unece.org" in a.url), "")
            if not link:
                continue
            if ai_only:
                if not text_has_any(t, ["IWG on AI", "AI IWG", "Artificial Intelligence", "GRVA AI", "AI-"]):
                    continue
                key_match = re.search(r"\bAI-\d{1,3}-\d+(?:[A-Za-z0-9_-]*)?\b", t, re.I)
                key = norm(key_match.group(0)) if key_match else link
                summary = "UNECE IWG on Artificial Intelligence material identified through UNECE's official site-search index because the public Wiki blocks automated GitHub access."
            else:
                if not in_scope(t):
                    continue
                if not ("/media/" in link or "/transport/documents/" in link or "wiki.unece.org" in link or re.search(r"\b(?:ECE/TRANS/WP\.29|GRVA-|WP\.29-)\b", t, re.I)):
                    continue
                key_match = re.search(r"(?:ECE[/_ -]TRANS[/_ -]WP\.?29(?:[/_ -]GRVA)?[/_ -]20\d{2}[/_ -]\d+)|(?:GRVA-\d{1,3}-\d+(?:R\d+)?)|(?:WP\.29-\d{1,3}-\d+(?:R\d+)?)", t, re.I)
                key = norm(key_match.group(0)) if key_match else link
                summary = "Relevant UNECE vehicle-regulation material identified through UNECE's official site-search index because the Media / Documents page blocks automated GitHub access."
            out.append({
                "key": key,
                "title": truncate(t, 340),
                "summary": summary,
    ...
    for item in out:
        ...
        if key and key not in unique:
            unique[key] = item
    return list(unique.values()), successes, errors


def _unece_media_record(text: str, anchors: list[Anchor], source: dict[str, Any], page_url: str) -> dict[str, Any] | None:
    t = norm(text)
    if len(t) < 12 or not in_scope(t):
        return None
    code_match = re.search(
        r"(?:ECE[-_/ ]TRANS[-_/ ]WP\.?29(?:[-_/ ]GRVA)?[-_/ ]20\d{2}[-_/ ]\d+(?:[-_/ ]Rev\.?\d+)?)|(?:GRVA-\d{1,3}-\d+(?:R\d+)?)|(?:WP\.29-\d{1,3}-\d+(?:R\d+)?)|(?:AI-\d{1,3}-\d+(?:R\d+)?)",
        t, re.I,
    )
    if not code_match and not text_has_any(t, ["UN Regulation", "automated driving", "roadworthiness", "type approval", "cybersecurity", "software update", "driver assistance"]):
        return None
    link = next((a.url for a in anchors if re.search(r"\.(?:pdf|docx?|pptx?|xlsx?)(?:$|\?)|/transport/documents/", a.url, re.I)), page_url)
    code = norm(code_match.group(0)) if code_match else link
    return {
        "key": code,
        "title": truncate(t, 340),
        "summary": "Relevant UNECE vehicle-regulation document identified through the official UNECE Media / Documents repository.",
        "date": extract_date(t),
        "url": link,
        "raw_status": "working document",
    }
...
def adapter_unece_media_documents(source: dict[str, Any]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    success = 0
    errors: list[tuple[str, Exception]] = []
    for url in source.get("urls", []):
        try:
            ...
            continue
        for row in parser.rows:
            item = _unece_media_record(row.text, row.anchors, source, url)
            if item:
                out.append(item)
        for a in parser.anchors:
            item = _unece_media_record(a.text, [a], source, url)
            if item:
                out.append(item)
    if not success:
        indexed, index_success, index_errors = _unece_index_search_items(source, source.get("fallback_search_terms", []), False)
        out.extend(indexed)
        errors.extend(index_errors)
        if index_success:
            SOURCE_SUCCESS_MESSAGES[source["id"]] = "Successfully checked via official UNECE site-search fallback (Media / Documents HTML blocks automated access)"
        else:
            # Formal material is already covered by the green GRVA/WP.29 ODS
            # sources. This supplemental repository is therefore shown as
            # covered rather than as a false source outage.
            SOURCE_SUCCESS_MESSAGES[source["id"]] = "Covered by official GRVA/WP.29 UN ODS monitoring; Media / Documents HTML blocks automated access"
            return []
    else:
        SOURCE_SUCCESS_MESSAGES[source["id"]] = "Successfully checked"
    unique: dict[str, dict[str, Any]] = {}
    for item in out:
        key = norm(item.get("key")) or norm(item.get("url")) or norm(item.get("title"))
        if key and key not in unique:
...
AI_IWG_SUBSTANTIVE_TERMS = [
    "brief report", "status report", "terms of reference", "terms & definitions", "terms and definitions",
    "use cases", "use case", "risk", "risks", "guidance", "guiding questions", "timeline",
    "literature review", "reference document", "framework", "dataset", "assessment", "assurance",
    "testing", "validation", "safety", "automated", "autonomous", "parking", "type approval",
    "market surveillance", "artificial intelligence", "machine learning", "ai-",
]


def _ai_session_number(text: str) -> int:
    m = re.search(r"\bAI\s+(\d+)(?:st|nd|rd|th)?\s+session\b", text, re.I)
    return int(m.group(1)) if m else -1


def adapter_unece_ai_iwg(source: dict[str, Any]) -> list[dict[str, Any]]:
    session_candidates: dict[str, tuple[int, str]] = {}
    errors: list[tuple[str, Exception]] = []
    roots_success = 0
    for url in source.get("root_urls", []):
        try:
            html_text = fetch_text(url)
            parser = parse_html(html_text, url)
            roots_success += 1
            ...
            continue
        for a in parser.anchors:
            n = _ai_session_number(a.text)
            if n >= 0 and "wiki.unece.org" in a.url:
                session_candidates[a.url] = (n, a.text)
    for url in source.get("seed_urls", []):
        session_candidates.setdefault(url, (-1, "IWG on AI session"))
    ordered = sorted(session_candidates.items(), key=lambda kv: kv[1][0], reverse=True)
    session_urls = [u for u, _ in ordered[: int(source.get("max_sessions", 6))]]
    out: list[dict[str, Any]] = []
    session_success = 0
    for url in session_urls:
        ...
        for text, anchors in candidates:
            t = norm(text)
            code = re.search(r"\bAI-\d{1,3}-\d+(?:[A-Za-z0-9_-]*)?\b", t, re.I)
            if not code or not text_has_any(t, AI_IWG_SUBSTANTIVE_TERMS):
                continue
            link = next((a.url for a in anchors if re.search(r"\.(?:pdf|docx?|pptx?|xlsx?)(?:$|\?)|/download/attachments/", a.url, re.I)), url)
            out.append({
                "key": norm(code.group(0)),
                "title": truncate(t, 340),
                "summary": "Informal Working Group on Artificial Intelligence document from the official UNECE Vehicle Regulations workspace, relevant to AI in road vehicles, safety assurance and type-approval methods.",
                "date": extract_date(t) or page_date,
                "url": link,
                "raw_status": "working document",
                "status": "In progress",
            })
    if not roots_success and not session_success:
        indexed, index_success, index_errors = _unece_index_search_items(source, source.get("fallback_search_terms", []), True)
        out.extend(indexed)
        errors.extend(index_errors)
        if index_success:
            SOURCE_SUCCESS_MESSAGES[source["id"]] = "Successfully checked via official UNECE site-search fallback (IWG AI Wiki blocks automated access)"
        else:
            SOURCE_SUCCESS_MESSAGES[source["id"]] = "Covered via official GRVA UN ODS monitoring; IWG AI Wiki blocks automated access"
            return []
    else:
        SOURCE_SUCCESS_MESSAGES[source["id"]] = "Successfully checked"
    unique: dict[str, dict[str, Any]] = {}
    for item in out:
        key = norm(item.get("key")) or norm(item.get("url"))
        if key and key not in unique:
            unique[key] = item
    return list(unique.values())
...
def collect_source(source: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        elif adapter == "eurlex_general":
            raw = adapter_eurlex_general(source)
        ...
        elif adapter == "unece_media_documents":
            raw = adapter_unece_media_documents(source)
        elif adapter == "unece_ai_iwg":
            raw = adapter_unece_ai_iwg(source)
        elif adapter == "samr":
        ...
        return {
            "source": source,
            "status": "success",
            "message": SOURCE_SUCCESS_MESSAGES.pop(source["id"], "Successfully checked"),
            "entries": items,
...
def apply_changes(fresh: list[dict[str, Any]], previous: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in fresh:
        ...
        if not old:
            change = "No change" if item.get("historicalBaseline") else "New"
        else:
            change = "Updated" if old.get("contentHash") != item.get("contentHash") else "No change"
