...
def adapter_unece_official(source: dict[str, Any]) -> list[dict[str, Any]]:
    """UNECE monitoring through official UN machine-readable systems.

    Direct unece.org pages can return HTTP 403 to GitHub-hosted runners. The
    adapter therefore combines the UN Digital Library API with the United
    Nations Official Document System (ODS). The UNECE Wiki remains an optional
    supplement for GRVA informal work and never determines source availability.
    """
    out: list[dict[str, Any]] = []
    primary_successes = 0
    errors: list[str] = []

    # 1) UN Digital Library API (JSON first, reduced MARCXML second)
    for spec in source.get("search_patterns", []):
        pattern = spec.get("pattern", "")
        field = spec.get("field", "")
        try:
            out.extend(_undl_search(source, pattern, field))
            primary_successes += 1
        except Exception as exc:
            errors.append(f"UN Digital Library ({pattern}): {norm(exc)}")

    # 2) Official UN ODS daily release lists, with a long enough look-back to
    # include the most recent WP.29/GRVA formal cycle.
    try:
        ods_items, successful_days, ods_errors = _ods_recent_items(source)
        out.extend(ods_items)
        if successful_days:
            primary_successes += 1
        elif ods_errors:
            errors.append("UN ODS: " + " | ".join(ods_errors[:2]))
    except Exception as exc:
        errors.append("UN ODS: " + norm(exc))

    # 3) Optional official UNECE Wiki for informal GRVA work.
    ...
    if not primary_successes:
        raise RuntimeError("Official UNECE/UN document systems could not be retrieved. " + " | ".join(errors[-4:]))
