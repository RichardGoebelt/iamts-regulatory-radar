...
EURLEX_GENERAL_TERMS = [
    ...
    "automated driving system", "automated vehicle", "advanced driver assistance", "driver assistance",
...
def adapter_eurlex_general(source: dict[str, Any]) -> list[dict[str, Any]]:
    try:
        return _eurlex_general_via_sparql(source)
    except Exception as sparql_exc:
        try:
            return _eurlex_general_via_search_pages(source)
        except Exception as search_exc:
            raise RuntimeError(f"EUR-Lex legislation monitoring failed — CELLAR: {norm(sparql_exc)} | EUR-Lex search: {norm(search_exc)}") from search_exc
    ...
    for url in source.get("urls", []):
        ...
        for a in parser.anchors:
            ...
            if item:
                out.append(item)
    if not success:
        raise RuntimeError("UNECE Media / Documents pages could not be retrieved. " + error_summary(errors))
    unique: dict[str, dict[str, Any]] = {}
    for item in out:
        key = norm(item.get("key")) or norm(item.get("url")) or norm(item.get("title"))
        if key and key not in unique:
...
def adapter_unece_ai_iwg(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not session_urls and not roots_success:
        raise RuntimeError("UNECE IWG on AI workspace could not be retrieved. " + error_summary(errors))
    ...
    if not session_success and not roots_success:
        raise RuntimeError("UNECE IWG on AI session pages could not be retrieved. " + error_summary(errors))
    unique: dict[str, dict[str, Any]] = {}
    for item in out:
